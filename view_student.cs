    ...
    public partial class view_student : Form
    {
        ...
        private void LoadReport() //ສ້າງຟັງຊັນໂຫຼດລາຍງານຈາກຖານຂໍ້ມູນ
        {
            try
            {
                ...
                Student_all rpt = new Student_all();// ຊື່ລາຍງານທີ່ສ້າງ
                rpt.SetDataSource(ds.Tables["tb_student"]);
        ...
        private void SearchAndLoadReport() //ສ້່າງຟັງຊັນຄົ້ນຫາຂໍ້ມູນໃນລາຍງານ
        {
            try
            {
                ...
                string sql = "SELECT * FROM tb_student WHERE s_id LIKE @search OR Name LIKE @search";//ປ້ອງກັນ SQL Injection
                SqlCommand cmd = new SqlCommand(sql, con);
